...
key("Set Window Size", function(next, driver, w, h) {
...
key("Close", function(next, driver) {
    driver.close().then(next);
});

key("Switch To First Tab", function(next, driver) {
...
key("Switch To Next Tab", function(next, driver) {
    var allHandles;
...
key("Wait For Element Content To Be", function(next, driver, selector, expectedContent) {
...
key("Has Element Class?", function(next, driver, selector, className) {
...
key("Is Element Present?", function(next, driver, selector) {
    driver
        .isElementPresent(driver.webdriver.By.css(selector))
        .then(next);
});

key("Sleep", function(next, driver, ms) {
...
key("Is Visible?", function(next, driver, selector) {
...
key("Press Button", function(next, driver, selector) {
...
key("Go To Page", function(next, driver, location) {
    driver.get(location).then(next);
});

key("Refresh", function(next, driver) {
    getHref(driver)
        .then(function(href) {
...
key("Quit", function(next, driver) {
    driver.quit().then(next);
});

key("Wait For Hash Change", function(next, driver) {
...
key("Reload Page", function(next, driver) {
    driver
        .navigate()
        .refresh()
        .then(next);
