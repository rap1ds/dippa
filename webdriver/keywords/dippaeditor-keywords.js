...
key("Get Preview ID", function(next, driver) {
    driver.executeScript(function() {
        return window.require('app/session').previewId;
    }).then(next);
});

key("Go To Line", function(next, driver, lineNum) {
...
key("Insert", function(next, driver, text) {
...
key("Get Editor Value", function(next, driver) {
    driver.executeScript(function() {
...
key("Set Editor Value", function(next, driver, value) {
...
key("Should Be Valid Id", function(next, driver, id) {
    debug("Checking validity of id", id);
    assert(/^[A-Za-z0-9_\-]{6,}$/.test(id), "Is invalid id");
    next();
});

key("Get Id From URL", function(next, driver) {
    getPath(driver)
        .then(function(path) {
            return path.split('/')[1];
        }).then(next);
...
key("Wait For Editor Initialized", function(next, driver) {
